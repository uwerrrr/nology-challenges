...
export const getIsValidOnNameTag = () => {
  return (
    ...
    // check the first letter with its own uppercase version
...
export const convertStringToNumber = () => {
  return parseFloat(stringToConvert); //parseFloat convert string to doubles.
};
...
export const getHasUppercaseLetters = () => {
  const regex = /[A-Z]/; // Regex pattern to match uppercase letters

  return regex.test(stringWithUppercaseLetters); // test whether a string matches a specified regular expression pattern. It returns true if the pattern is found in the string, and false otherwise.
};
...
export const convertPascalCaseToSnakeCase = () => {
  const makeFirstLetterLowerCase =
    ...
    pascalCaseVariableName.slice(1); // = iWantToBeSnakeCase

  // replace all Uppercase letters with "_[its self]"
  const regex = /[A-Z]/g;
  let result = makeFirstLetterLowerCase.replace(regex, "_$1");
